...
import { PlayerCreateData, TournamentCreationData, PlayerData, PlayersCreateData, PlayerModificationCreateData } from "./player.types";
...
export const createPlayer = asyncWrapper(async (req: RequestWithUserId, res: Response): Promise<void> => {
    ...
    // //const tournamentTypeId = await tournamentService.getTournamentTypeId()
...
export const insertOrRemoveTournamentPlayer = asyncWrapper(async (req: RequestWithUserId, res: Response): Promise<void> => {
    const creationData = req.body as PlayerModificationCreateData;
    const createdPlayerModificationId = await playerService.createPlayerModification(creationData);
    const resBody: ResBody<number> = {
        success: true,
        data: createdPlayerModificationId
    };
    res.status(StatusCodesOkay.Created).json(resBody);
});
